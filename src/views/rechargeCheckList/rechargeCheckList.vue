 ...
 <div class="pad_5">
    ...
    <div>
      ...
      <el-table :data="tableData" v-loading="tableLoading" border stripe style="width: 100%">
          ...
          <!-- 现有余额 -->
          <el-table-column prop="accountRemain" label="现有余额(元)" width="110" >
          </el-table-column>
          <!-- 充值金额 -->
          <el-table-column prop="recharge_money" label="充值金额(元)" width="110" >
          </el-table-column>
          <!-- 创建时间 -->
          <el-table-column prop="rechargeTime" show-overflow-tooltip label="创建时间" width="120" >
          </el-table-column>
          <!-- 操作 -->
    ...
    <el-dialog
      ...
      <el-form :inline="true" :model="detail_check_form" :rules="detail_check_rules" ref="detail_check_form"  class="demo-ruleForm demo-form-inline valid_form">
         <el-form-item>
           <span>{{detail_check_form.agent_name}}</span>
           <span>红包充值</span>
           <span>{{detail_check_form.recharge_money}}元</span>
         </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getTableDataList(pageNum){
      ...
      this.$http.post(`${ commonUrl.baseUrl }/agent/agentRechargeCheckList`, param).then(res=>{
        if(res.data.code == '0000'){
    ...
    handle_check(row){
      ...
      this.$http.post(`${commonUrl.baseUrl}/agentRechargeCheck/rechargeCheckInfo`, param).then(res=>{
        if(res.data.code == '0000'){
          // 取agent_name rechargeMoney
          let result = res.data.data.agentRechargeCheckInfo
          this.detail_check_form.recharge_money = result.rechargeMoney
          this.detail_check_form.agent_recharge_checkid = result.agent_recharge_checkid
      ...
      if(this.m_valid_addForm('detail_check_form')){
        let param = {
            data: {
                ...
                // 私参
                recharge_money:this.detail_check_form.recharge_money*100,
                agent_recharge_checkid:this.detail_check_form.agent_recharge_checkid,
                agentid:this.detail_check_form.agentid,
                recharge_check_status:this.detail_check_form.merchant_check
            }
        }
        this.detail_check_loading = true
        this.$http.post(`${ commonUrl.baseUrl}/agentRechargeAccount/rechargeCheck`, param).then(res=>{
            if(res.data.code == '0000'){
