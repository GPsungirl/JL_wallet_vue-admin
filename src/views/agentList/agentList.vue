<template>
    ...
    <div class="pad_5">
            ...
            <el-table :data="tableData" v-loading="tableLoading" border stripe style="width: 100%">
                ...
                <el-table-column prop="" label="状态" width="">
                    <template slot-scope="scope">
                        ...
                        <span v-else-if="scope.row.agent_status == 2">不可用</span>
                    </template>
                </el-table-column>
                <!-- 审核状态 -->
                ...
                <!-- 会员验证状态 -->
                <el-table-column v-if="roleId == 1" prop="" label="会员验证状态" width="">
                    <template slot-scope="scope">
                        <span v-if="scope.row.enable_member == 0">关闭</span>
                        <span v-else-if="scope.row.enable_member == 1">开启</span>
                    </template>
                </el-table-column>
                ...
                <template slot-scope="scope">
                    ...
                    <!-- 冻结和解冻 1是可用状态；2是不可用状态-->
                    <el-button v-if="scope.row.agent_status == 1" @click="handle_frozen(scope.row)" type="text" size="small">冻结
                    </el-button>
                    <el-button v-if="scope.row.agent_status == 2" @click="handle_frozen(scope.row)" type="text" size="small">解冻
                    </el-button>
                    <!-- 只有运营  有权限 修改 roleId == 1-->
                    <el-button v-if="roleId == 1 && scope.row.agent_type == 0" @click="handle_detail_check(scope.row)" type="text" size="small">修改</el-button>
                    <!-- 开启/关闭会员验证：超管roleId=1;审核通过agent_check=2; 机构是否可用agent_status=1 -->
                    <el-button v-if="roleId == 1 && scope.row.agent_check == 2 && scope.row.agent_status==1 && scope.row.enable_member == 0" @click="handle_updateMember(scope.row)" type="text" size="small">开启会员验证</el-button>
                    <el-button v-if="roleId == 1 && scope.row.agent_check == 2 && scope.row.agent_status==1 && scope.row.enable_member == 1" @click="handle_updateMember(scope.row)" type="text" size="small">关闭会员验证</el-button>
                </template>
                </el-table-column>
            </el-table>
            <!-- 分页 -->
        ...
        <el-dialog
            ...
            >
            <!-- 业务信息 -->
            <el-form :inline="true" :model="detail_form" label-width="80px"  class="demo-form-inline " disabled>
                <!-- 业务信息 -->
                ...
                <el-form-item label="交易分成" prop="commission_proportion">
                    <el-input v-model="detail_form.commission_proportion" placeholder="交易分成" class="wid_181"></el-input>%
        ...
        <el-dialog
            ...
            <el-form
                ...
                <el-form-item label="开户分成" prop="active_fee">
                    <el-input v-model="detail_check_form.active_fee" placeholder="开户分成" class="wid_181"></el-input>元
...
import {isvalidPhone, validNum100, filterSpace,validNum2,validNum20} from '../../utils/validate'
export default {
    name: 'agentList',
    data(){
        // 校验手机号
    ...
    methods:{
        ...
        getTableDataList(pageNum){
          ...
          this.$http.post(`${ commonUrl.baseUrl }/agent/selectAgentList`, param).then(res=>{
              if(res.data.code == '0000'){
        ...
        handle_detail_check(row){
            ...
            this.$http.post(`${ commonUrl.baseUrl }/agent/updateAgent`, {data:{agentid:row.agentid}}).then(res=>{
                if(res.data.code == '0000'){
                    ...
                    // 交易分成
                    this.detail_check_form.commission_proportion = result.commissionProportion*10;
                    // 开户分成
              ...
              this.detail_check_form.account_no = filterSpace(this.detail_check_form.account_no)
                let param = {
                    data:{
                        ...
                        commission_proportion:this.detail_check_form.commission_proportion*10,
                        active_fee:this.detail_check_form.active_fee*100,
                        ...
                        account_user_name:this.detail_check_form.account_user_name,
        ...
        handle_frozen(row) {
          ...
          if(row.agent_status== 1){
            dialog_title="冻结"
            dialog_content="是否冻结该机构?"
          }else{
            dialog_title="解冻"
            dialog_content="是否解冻该机构?"
          }
          this.$confirm(dialog_content, dialog_title, {
            ...
            this.$http
              .post(`${commonUrl.baseUrl}/agent/updateAgentStatus`, param)
              .then(res => {
                if (res.data.code == "0000") {
                  loading.close();
                  this.m_message(res.data.msg, "success");
        ...
        // 主列表开启/关闭会员验证
        handle_updateMember(row){
          let dialog_title = ""
          let dialog_content =""
          if(row.enable_member== 0){ //当前处于关闭状态
            dialog_title="开启会员验证"
            dialog_content="是否开启会员验证?"
          }else{
            dialog_title="关闭"
            dialog_content="是否关闭会员验证?"
          }
          this.$confirm(dialog_content, dialog_title, {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            ...
            this.$http
              .post(`${commonUrl.baseUrl}/agent/updateEnableMember`, param)
              .then(res => {
                if (res.data.code == "0000") {
                  loading.close();
                  this.m_message(res.data.msg, "success");
